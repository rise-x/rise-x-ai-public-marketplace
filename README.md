...
2. **Enable auto-update.** Open `/plugin` → **Marketplaces** →
   ...
   never receive another version of either plugin. Details and the manual
   alternative: [Keeping plugins up to date](#keeping-plugins-up-to-date).
3. **`/rise-x-mcp:setup`** — signs you in to the two Rise-X MCP servers
   ...
   below for the full walkthrough, and
...
Recent Claude Code versions activate each plugin as part of the install and
say so (`Plugin is now active.`); if the summary says `Run /reload-plugins to
activate.` instead, run that and the plugin's skills and MCP servers load
into the current session, with no restart needed.
...
- **Organization-managed (zero-touch):** admins can register this marketplace
  ...
  **Organization settings → Plugins** instead requires a private or internal
  mirror of this repository (see [Install](#install)); members then pick up
...
[Install](#install) above — slash commands in a terminal session, or the
plugin browser in a desktop Code session.
...
for the full walkthrough, and `plugins/rise-x-mcp/skills/setup/SKILL.md` for
troubleshooting OAuth and authorization failures.
