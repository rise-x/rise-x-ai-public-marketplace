...
9. **A GUID literal in app source is a bug.** Flow and asset-type origin ids belong in `rise-x-app.json` at the app project root — one alias per target, ids per environment — and app code reads them through `useAppDependencies()` (`deps.<alias>`, SDK ≥ 0.10.0). Only flows the *user* picks at runtime go through the generic connectors with a ref (`references/build.md` §App dependencies).
