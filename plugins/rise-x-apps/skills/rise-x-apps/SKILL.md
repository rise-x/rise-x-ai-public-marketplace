...
9. **A GUID literal in app source is a bug.** Flow and asset-type origin ids and agent ids belong in `rise-x-app.json` at the app project root — one alias per target, ids per environment — and app code reads them through `useAppDependencies()` (`deps.<alias>`, SDK ≥ 0.12.0). An agent's id field is `agentId`, not `flowOriginId`. Only flows the *user* picks at runtime go through the generic connectors with a ref (`references/build.md` §App dependencies).
