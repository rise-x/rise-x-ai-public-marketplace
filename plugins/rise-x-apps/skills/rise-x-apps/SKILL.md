...
9. **A flow, asset-type, or agent GUID in app source is a bug.** Those ids belong in `rise-x-app.json` at the app project root — one alias per target, ids per environment — and app code reads them through `useAppDependencies()` (`deps.<alias>`; needs a recent `@rise-x/apps-sdk` — `references/build.md` §App dependencies names the version). An agent's id field is `agentId`, not `flowOriginId`. Declare only the environments you have real ids for — the file starts at `["test"]` — because every dependency needs an id for **every** environment it declares; ask the user (or discover per environment over the MCP) rather than inventing, copying, or guessing an id to fill a gap. Run `pnpm validate` after editing the file and before any deploy, since a build only checks the one environment it targets; it also warns about ids still hardcoded in `src/`, and those warnings don't affect its exit code, so read them. `npx @rise-x/apps-sdk scan` finds hardcoded ids on its own — reach for it first when migrating an old app, instead of hunting through the code for GUIDs. Task and step ids are the exception: the manifest doesn't carry them, so they stay in app config. **A declared dependency is the recommended way to reach any Rise-X object the app knows at authoring time**, because `deps.<alias>` also carries the connector operations that take that ref, pre-filled. Use a generic connector with a ref only for what a dependency can't express: flows the *user* picks at runtime, discovery (`flows.list`, `assets.types`), and the operations no bound surface carries. `getShellApiV4` is last, for endpoints no connector covers (`references/build.md` §App dependencies, §Runtime APIs).
