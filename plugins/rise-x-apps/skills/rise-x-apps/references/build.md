...
import {
  ...
  vectorStores, // SDK >= 0.14.0, file-search corpora for agent runs, see below
...
| `vectorStores` | file-search corpora for `agents.run` / `chat.send` (SDK >= 0.14.0) | `create`, `get`, `addFiles`, `listFiles`, `renew`, `rename`, `rebuild`, `deleteFile`, `delete` |
...
// → getChatMessages(chatId) for the full transcript. Every run is logged.
...
All connector failures normalize to `ConnectorError` (`code: 'SHELL_UNAVAILABLE' | 'SHELL_TOO_OLD' | 'AI_UNAVAILABLE' | 'HTTP_ERROR' | 'NOT_FOUND' | 'NETWORK_ERROR' | 'ABORTED' | 'PARSE_ERROR' | 'INVALID_ARG'`); `INVALID_ARG` is the client-side argument check — an empty id, a blank title, an empty `vectorStoreIds` entry — thrown before any network call. `agents.run`/`createChat` need bridge v3 (`getAi`) and an environment with the AI gateway enabled — handle `SHELL_TOO_OLD`/`AI_UNAVAILABLE` gracefully. Every `vectorStores` method needs the same bridge: `SHELL_TOO_OLD`/`AI_UNAVAILABLE` fire before the call reaches any HTTP status. Chat memory is server-persisted and every run is logged: the gateway creates a chat whenever the call carries no `chatId` and reports its id on `end_of_stream`, so an app cannot opt out of the transcript (`useHistory` is gone as of SDK 0.14.0 — delete the argument wherever you meet it, `useHistory: false` included: the gateway ignores the flag, so an app still on 0.13 is already having every run logged and deleting it changes nothing). Clients only ever handle `chatId`: pass it back to continue that conversation, or ignore it for a fire-and-forget answer. A run that dies before a chat is resolved reports `chat_id: null` on the raw `end_of_stream` frame, which leaves `chat.chatId` (and `snapshot.chatId`) unset rather than null — test for absence (`if (!chat.chatId)`), never for `null`. `listChats`/`getChat`/`renameChat`/`deleteChat`/`getChatMessages` read the chat store at `/api/v4/ai/agent-chat`.

**File search for a run (`vectorStores`, SDK >= 0.14.0).** Build a corpus with `vectorStores.create({ name, expiresAfterDays? })`, add files with `vectorStores.addFiles(id, { files })`, then poll `vectorStores.get(id)` until `fileCounts.inProgress` is 0 before running an agent against it. `addFiles` posts the browser `File` objects straight to the gateway under one multipart `files` key, each with its own `file.name` — there is no staging step to run first, unlike the MCP tools' `request_vector_store_upload` → PUT → `add_vector_store_files` flow, which stages because MCP can't carry bytes. Acceptance is decided per file by extension: pdf, doc(x), pptx, txt, md, json, html, css, tex and common code files are searchable as-is; xlsx, xls, csv and eml are converted to Markdown; images, msg, rtf and odt are rejected outright with no conversion path. A zip is expanded server-side. The MCP-side `references/managing-vector-stores.md` in the `rise-x-mcp` plugin carries the full format table and the per-file traps. For a corpus scoped to this conversation or work item, pass the id on the run itself: `agents.run({ agentId, message, vectorStoreIds: [id] })` or `chat.send(message, { vectorStoreIds: [id] })` — at most 5 `vectorStoreIds` per run; a sixth is rejected as a validation error (422). An agent's own `file_search` config is for a corpus every user of that agent should share, not a per-run one. The store expires `expiresAfterDays` days (default 90, max 365 — outside that range is a 400) after its last use, sliding forward on every agent run that searches it; a `file_search` call alone doesn't move the clock, so a corpus nobody runs against expires on schedule. Confirm that number with the user before creating one. Save the returned `id` on the work item or asset it belongs to (for example `work.patchData`) right away: there is no list call to find it again later, and `expiryNotice` on the returned `VectorStore` is what to show next to the upload UI once the store nears expiry. `vectorStores.renew(id, { expiresAfterDays })` requires `expiresAfterDays`; `vectorStores.rebuild(id, { expiresAfterDays? })` does not — both return the store, and `rebuild`'s is a new id that needs saving in that same place. `addFiles` resolves with each file's `status: 'accepted' | 'converted' | 'rejected' | 'failed'` plus `reason`/`lastError` — a call that resolves successfully can still carry rejected or failed files, so surface those to the user rather than treating a resolved promise as "all files indexed". Errors normalize the same way as elsewhere: 404 to `NOT_FOUND`, other statuses to `HTTP_ERROR` with the server's detail (409 renewing after expiry or rebuilding before it, 410 rebuilding with no files left to rebuild from, 413 over the 100 MB upload limit, 422 running an agent against an expired or missing store — the failure the poll-until-`inProgress`-is-0 advice above prevents).
