...
All connector failures normalize to `ConnectorError` (`code: 'SHELL_UNAVAILABLE' | 'SHELL_TOO_OLD' | 'AI_UNAVAILABLE' | 'HTTP_ERROR' | 'NOT_FOUND' | 'NETWORK_ERROR' | 'ABORTED' | 'PARSE_ERROR' | 'INVALID_ARG'`). `agents.run`/`createChat` need bridge v3 (`getAi`) and an environment with the AI gateway enabled — handle `SHELL_TOO_OLD`/`AI_UNAVAILABLE` gracefully. Every `vectorStores` method needs the same bridge: `SHELL_TOO_OLD`/`AI_UNAVAILABLE` fire before the call reaches any HTTP status. Chat memory is server-persisted and every run is logged — the gateway creates a chat whenever the call carries no `chatId` and reports its id on `end_of_stream`, so there is no one-shot mode and clients only ever handle `chatId`: pass it back to continue that conversation, ignore it for a fire-and-forget answer, and check it for null, which a run that died before a chat was resolved still reports. `listChats`/`getChat`/`renameChat`/`deleteChat`/`getChatMessages` read the chat store at `/api/v4/ai/agent-chat`.
