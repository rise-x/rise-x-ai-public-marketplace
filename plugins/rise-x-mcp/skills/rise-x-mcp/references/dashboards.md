...
- `get_dashboard(id, format="summary"|"full")` — summary (the default) returns identity, filters, `dataSource` flattened to its name, and one line per component; `format="full"` adds complete component properties (incl. `reportData`) and `columnsData`. Access lists (`users`/`companies`) are summarized to counts in both formats.
...
`get_dashboard` returns the configuration back under the same singular `dataSource` field (the default summary format flattens it to just the name string; `format="full"` returns the `{name, properties}` object). Setting `data_source` requires Owner / Editor / environment-owner on the dashboard, matching the gate on `defaultQueryFilters`.

**If a dashboard returns 400 when fetching aggregated data:** check `get_dashboard(id).dataSource` (the name directly in the default summary format; `.dataSource.name` with `format="full"`). If it's anything other than `"Aggregate"` (or `"works"`/unset for non-aggregated), patch with `update_dashboard_metadata(id, data_source={"name": "Aggregate", "properties": {}})`. The aggregated rendering and each tile's `reportData` are unchanged — only the dispatch label is.
...
- **Read it:** `get_dashboard(id, format="full")` returns `dataSource` at the dashboard root and components with their `reportData` blocks — the default summary format projects components to one-line stubs without `reportData`.
