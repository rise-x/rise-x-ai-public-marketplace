...
**Verification status:** `relatedWork` rendering and the publish-mode pinning were verified in-app; `tabs` replace-not-merge, `navigation` / `contents` / `defaultTab` / `label`, and the no-validation behaviour are confirmed from the panel implementation but have not been exercised end-to-end via MCP.
