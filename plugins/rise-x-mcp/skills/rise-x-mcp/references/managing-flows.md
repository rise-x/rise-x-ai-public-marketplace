...
**Per-tab `label`** (optional string) overrides the tab's translated caption; omit it for the default name. An empty string is honoured and renders a blank caption — omit the key instead. The `navigation` tab additionally accepts grouping/sorting/search config on its entry (not documented here; unverified via MCP).
...
**Verification status:** the full config shape — all five tab values (including `navigation` / `contents`), per-tab `label`, and `defaultTab` — was written via MCP on a draft, published, and read back intact on a test ecosystem. `relatedWork` rendering and the publish-mode pinning were verified in-app. `tabs` replace-not-merge and the no-validation / broken-tab rendering behaviour are confirmed from the panel implementation (rendering of `navigation` / `contents` not visually re-checked).
