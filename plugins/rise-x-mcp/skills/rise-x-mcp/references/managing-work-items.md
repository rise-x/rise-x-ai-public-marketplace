...
> ⚠️ **`search_works` + `data.*` requires `flowOriginId`.** Whenever the request touches ANY `data.*` path (in `filter`, `sort`, OR `fields`) the same `filter` MUST contain a `flowOriginId` leaf — `equals` for one flow, `in` for multiple — under the same `and` group. Without it the server returns 400. Discover the valid `data.*` paths first via `get_flow_data_schema(flow_origin_id)`. The only exception is the bare `"data"` projection key (no dot) which skips schema resolution. On Work, `contains` and `endsWith` are **not available at all** — they are Flow-resource-only, so Work string fields (both the static whitelist and `data.*`) accept only `equals`/`notEquals`/`in`/`notIn`/`startsWith`. Full wrong-vs-right examples in `references/advanced-search.md`.
