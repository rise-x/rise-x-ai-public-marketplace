...
//
// The directory entry is flushed after the rename, the way every other write
// in the kit is: without it the swap lives in the directory cache, and a
// machine that loses power here can come back with neither the old binary nor
// the new one. Past the rename the update is installed, so a failed flush is
// a durability warning rather than a failed update, and says so.
